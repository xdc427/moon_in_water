...
#include"structs_x_shadow2.h"
#include<stdio.h>

static char buffer_x_shadow2[ sizeof( struct_all_s ) + ( sizeof( struct_one ) + sizeof( struct_one_s ) ) * 15 + sizeof( var_s ) * 2 ] = { 0 };
...
struct_all shadow_struct_x_shadow2_init()
{
	...
	struct_head = ( struct_all )buffer_x_shadow2;
	struct_head->size = sizeof( buffer_x_shadow2 );
	struct_head->num = 15;
	...
	*one_arr++ = tmp_one;
	...
	snprintf( tmp_one->name , sizeof( tmp_one->name ), "\\blist_shadow2_s\\b" );
	tmp_one->size = sizeof( struct list_shadow2_s );
	tmp_one->num = 2;
	tmp_one->point_num = -1;
	...
	*one_arr++ = tmp_one;
	...
	tmp_var->size = sizeof( ( ( struct list_shadow2_s * )0 )->id );
	tmp_var->addr = &( ( ( struct list_shadow2_s * )0 )->id );
	tmp_var->array_d = 0;
	...
	snprintf( tmp_var->type, sizeof( tmp_var->type ), "struct list_shadow2_s *" );
	tmp_var->size = sizeof( ( ( struct list_shadow2_s * )0 )->next );
	tmp_var->addr = &( ( ( struct list_shadow2_s * )0 )->next );
	tmp_var->array_d = 0;
	tmp_var->array_elem_size = sizeof( struct list_shadow2_s * );
	tmp_var->len = tmp_var->size / tmp_var->array_elem_size;
	tmp_var->is_hide = 0;
...
{
	return sizeof( struct list_shadow2_s * );
}

inline unsigned long get_x_shadow2_head_len()
{
	return sizeof( head ) / sizeof( struct list_shadow2_s * );
}
