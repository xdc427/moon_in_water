...
#include"structs_test_code.h"
#include<stdio.h>

static char buffer_test_code[ sizeof( struct_all_s ) + ( sizeof( struct_one ) + sizeof( struct_one_s ) ) * 15 + sizeof( var_s ) * 2 ] = { 0 };
...
struct_all shadow_struct_test_code_init()
{
	...
	struct_head = ( struct_all )buffer_test_code;
	struct_head->size = sizeof( buffer_test_code );
	struct_head->num = 15;
	...
	*one_arr++ = tmp_one;
	...
	snprintf( tmp_one->name , sizeof( tmp_one->name ), "\\blist_s\\b" );
	...
	*one_arr++ = tmp_one;
	...
	tmp_var->addr = &( ( ( struct list_s * )0 )->id );
	tmp_var->array_d = 0;
	...
	tmp_var->addr = &( ( ( struct list_s * )0 )->next );
	tmp_var->array_d = 0;
	tmp_var->array_elem_size = sizeof( struct list_s * );
	tmp_var->len = tmp_var->size / tmp_var->array_elem_size;
	tmp_var->is_hide = 0;
...
{
	return sizeof( struct list_s * );
}

inline unsigned long get_test_code_head_len()
{
	return sizeof( head ) / sizeof( struct list_s * );
}
...
{
	return sizeof( data_point ) / sizeof( int * );
}
