...
STATIC_GET_INTERFACE( test_hub, timer_listener_interface_s, 0 ) = {
...
//		CALL_INTERFACE_FUNC( p_data1, timer_interface_s, stop, p_point_data1);
		CALL_INTERFACE_FUNC( p_data1, timer_interface_s, del, p_point_data1);
		CALL_INTERFACE_FUNC( p_data1, timer_interface_s, restart, p_point_data1);
	}else{
		CALL_INTERFACE_FUNC( p_data1, timer_interface_s, set, p_point_data1, &desc );
		CALL_INTERFACE_FUNC( p_data1, timer_interface_s, stop, p_point_data1);
		CALL_INTERFACE_FUNC( p_data1, timer_interface_s, restart, p_point_data1);
...
void main()
{
	...
	for( i = 0; i < 16000; i++ ){
		...
		p_test->p_timer_pipe->id = i;
		MOON_PRINT( TEST, NULL, "put timer:%d", i );
		desc.mark_id = i;
...
*/
	sleep( 30 );
//	CALL_INTERFACE_FUNC( p_test->p_timer_pipe, pipe_interface_s, close );
//	CALL_INTERFACE_FUNC( p_test->p_timer_pipe, gc_interface_s, ref_dec );
}
